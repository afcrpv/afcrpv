...
$.fn.check_show_indication = ->
  @each ->
    ...
    condition = /cypro|diane|androcur|evepar|holgyeme|lumalia|minerva/.test($select.find("option:selected").text())
    showNextif condition, $select, $next

    $select.on "change", ->
      condition = /cypro|diane|androcur|evepar|holgyeme|lumalia|minerva/.test($(@).find("option:selected").text())
